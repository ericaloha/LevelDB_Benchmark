  ...
  void Run() {
    ...
    while (benchmarks != NULL) {
      ...
      /*
        ...
        */
        else if (name == Slice("read&zipK")) {
        num_threads++;  // Add extra thread for writing
        method = &Benchmark::ReadWhileWritingZipK;
      } 
        else if (name == Slice("read&zipS")) {
        num_threads++;  // Add extra thread for writing
        method = &Benchmark::ReadWhileWritingZipS;
      }
        else if (name == Slice("read&Uni")) {
        num_threads++;  // Add extra thread for writing
...
*/
  ...
  void DoWrite_zipfian(ThreadState* thread, bool seq) {
    ...
    char filename[80];
    if(seq){
        snprintf(filename,sizeof(filename),"%s","/home/leveldb/LogDir/lantency_skewedZip.txt");
    }else{
        snprintf(filename,sizeof(filename),"%s","/home/leveldb/LogDir/lantency_ScrambledZip.txt");
    }

    struct timespec tpend;
    long timedif;
    std::ofstream openfile(filename,std::ios::app);
    openfile<<"------------------------------------------------------------------------------------------------------\n";
    

    if (num_ != FLAGS_num) {// Number of key/values to place in database
      char msg[100];
    ...
    generator::SkewedLatestGenerator slg(1000);//can be modified by need 
    generator::ScrambledZipfianGenerator szg(1000); //can be modified by need 
    WriteBatch batch;
    Status s;
    int64_t bytes = 0;
...
void DoWrite_Y_U(ThreadState* thread, bool seq) {
        //try to write lantency to file
        //some info can be inserted into the file or added into the filename
        struct timespec tpstart;
        struct timespec tpend;
        long timedif;
        std::ofstream openfile("/home/leveldb/LogDir/lantency_Uniform.txt",std::ios::app);
        openfile<<"------------------------------------------------------------------------------------------------\n";
    ...
    RandomGenerator gen;
    generator::UniformIntegerGenerator uig(1,9999);
	  //generator::YCSBKeyGenerator ycsb(1,100,5);//startnum, filenum, keysize can be modified by need 

    WriteBatch batch;
    Status s;
    int64_t bytes = 0;
    for (int i = 0; i < num_; i += entries_per_batch_) {//entries_per_batch_ was defined as 1 by default!!!!!! you can change depend on your need (fillbatch operation has changed which to 1000)
      batch.Clear();
      for (int j = 0; j < entries_per_batch_; j++) {
  ...
  void ReadWhileWritingZipK(ThreadState* thread) {
    ...
    if (thread->tid > 0) {
      ...
      generator::SkewedLatestGenerator slg(10000); //can be modified by need
      RandomGenerator gen;
      while (true) {
        {
...
void ReadWhileWritingZipS(ThreadState* thread) {
    ...
    if (thread->tid > 0) {
      ...
      generator::ScrambledZipfianGenerator szg(10000); //can be modified by need
      RandomGenerator gen;
      while (true) {
        {
          MutexLock l(&thread->shared->mu);
...
void ReadWhileWritingUni(ThreadState* thread) {
    ...
    if (thread->tid > 0) {
      ...
      generator::UniformIntegerGenerator uig(1,99999); //can be modified by need
      RandomGenerator gen;
      while (true) {
        {
          MutexLock l(&thread->shared->mu);
